    ...
    if (cartItems.length === 0) {
        return (
            <div className="pt-32 text-center min-h-screen bg-royal-cream">
                ...
                <button onClick={() => router.push('/shop')} className="mt-4 text-royal-gold font-medium">Continue Shopping</button>
            </div>
        )
    }
                                ...
                                {cartItems.map((item) => (
                                    <div key={item.id} className="flex justify-between items-start">
                                        <div className="flex space-x-3">
                                            <div className="w-12 h-12 bg-white rounded-md overflow-hidden border border-gray-100 relative">

...
export default function Inventory() {
    ...
    const handleEditClick = (item) => {
        setEditItem(item);
        setEditFormData({ stock: item.stock || 0, lowStockThreshold: item.lowStockThreshold || 10 });
    };
    ...
    const filteredInventory = inventory.filter(item => {
        ...
        if (filter === 'low') return (item.stock || 0) <= (item.lowStockThreshold || 10) && (item.stock || 0) > 0;
    ...
    const getStockStatus = (item) => {
        ...
        if (stock === 0) return { label: 'Out of Stock', color: 'bg-red-100 text-red-700 border-red-200', barColor: 'bg-red-500' };
        if (stock <= threshold) return { label: 'Low Stock', color: 'bg-orange-100 text-orange-700 border-orange-200', barColor: 'bg-orange-500' };
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {loading ? (
                                ...
                                filteredInventory.map(item => {
                                    ...
                                    const percentage = Math.min(100, ((item.stock || 0) / 100) * 100);

                                    return (
                                        <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                                            <td className="p-4">
                                                <div className="flex items-center space-x-3">
                                                    <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-royal-gold overflow-hidden border border-gray-200">
                                                        {item.image ? (
